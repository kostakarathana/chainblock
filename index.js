...
(function(){
  ...
  function mockPriceSeries(n, base){
    n = n || 200; base = base || 0.24 + (Math.random()-0.5)*0.04;
    var out = [];
    for (var i=0;i<n;i++){
      var t = i/10;
  ...
  function runBacktest(){
    ...
    var cash = 1000;     // in USDC
    var algo = 0;        // in ALGO units
    var trades = 0;

    for (var i=0;i<series.length;i++){
      var p = series[i];
      var trigger = (s.op === '<') ? (p < s.thr) : (s.op === '>') ? (p > s.thr) : (Math.abs(p - s.thr) < 1e-9);
      if (!trigger) continue;
    ...
    $('btSummary').innerHTML =
      '<div><strong>Trades</strong><br>' + trades + '</div>' +
      '<div><strong>Trigger</strong><br>price ' + s.op + ' ' + s.thr + '</div>' +
      '<div><strong>Action</strong><br>' + (s.side||'BUY') + ' ' + Math.round(s.pct*100) + '%</div>';
  }
  ...
  function drawChart(canvas, equity){
    ...
    var toX = function(i){ return pad + i*(w-2*pad)/(equity.length-1); };
    var toY = function(v){ return h-pad - (v-min)*(h-2*pad)/((max-min) || 1); };

    ctx.lineWidth = 2 * window.devicePixelRatio;
    ctx.beginPath();
    for (var i=0;i<equity.length;i++){
      var x = toX(i), y = toY(equity[i]);
  ...
  document.getElementById('btnReset').addEventListener('click', function(){ workspace.clear(); seedDefault(); });
